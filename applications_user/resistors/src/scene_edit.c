...
static bool widget_input_callback(InputEvent* input_event, void* context) {
    ...
    if(input_event->type == InputTypeShort) {
        ...
        case InputKeyDown:
            ...
            break;
        case InputKeyOk:
            // T-Embed encoder shortcut: short-click OK cycles to the next band
            // (wrapping after the last), so you never need the hold-and-turn
            // gesture to reach Left/Right. resistor_type == band count (R3=3..R6=6).
            app->state->edit_selection =
                (app->state->edit_selection + 1) % app->state->resistor_type;
            consumed = true;
            break;
        default:
            consumed = false;
