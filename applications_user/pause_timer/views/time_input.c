 ...
 * skipping the spanned continuation cells of the wide START/LEARN keys
 * (width==0). Left/Right keep their row/column behaviour for modifier users. */
static void time_input_step_linear(TimeInputModel* model, int dir) {
    const int total = ROW_COUNT * COLUMN_COUNT;
    for(int guard = 0; guard < total; guard++) {
        int idx = (model->y * COLUMN_COUNT + model->x + dir + total) % total;
        model->x = idx % COLUMN_COUNT;
        model->y = idx / COLUMN_COUNT;
        if(time_input_keyset[model->y][model->x].width != 0 &&
           time_input_keyset[model->y][model->x].value != -1)
            return;
    }
}
