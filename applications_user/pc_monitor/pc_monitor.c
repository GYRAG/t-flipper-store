...
int32_t pc_monitor_app(void* p) {
    ...
    /* This port's serial profile ignores per-app params (it advertises with a
     * fixed device name), so there is no BleProfileSerialParams to pass. */
    app->ble_serial_profile = bt_profile_start(app->bt, ble_profile_serial, NULL);
