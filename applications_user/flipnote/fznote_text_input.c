...
typedef struct {
    const FzNoteTextInputKey* rows[3];
...
typedef struct {
    ...
    FzNoteTextInputValidatorCallback validator_callback;
...
static const FzNoteTextInputKey keyboard_keys_row_1[] = {
    {'q', 1, 8},
    {'w', 10, 8},
    {'e', 19, 8},
    {'r', 28, 8},
...
static const FzNoteTextInputKey keyboard_keys_row_2[] = {
    {'a', 1, 20},
    {'s', 10, 20},
    {'d', 19, 20},
    {'f', 28, 20},
...
static const FzNoteTextInputKey keyboard_keys_row_3[] = {
...
static const FzNoteTextInputKey symbol_keyboard_keys_row_1[] = {
    {'!', 2, 8},
    {'@', 12, 8},
    {'#', 22, 8},
    {'$', 32, 8},
...
static const FzNoteTextInputKey symbol_keyboard_keys_row_2[] = {
    {'~', 2, 20},
    {'+', 12, 20},
    {'-', 22, 20},
    {'=', 32, 20},
...
static const FzNoteTextInputKey symbol_keyboard_keys_row_3[] = {
    {SWITCH_KEYBOARD_KEY, 1, 23},
    {'.', 15, 32},
    {',', 29, 32},
    {':', 41, 32},
...
static void switch_keyboard(FzNoteTextInputModel* model) {
...
static const FzNoteTextInputKey* get_row(const Keyboard* keyboard, uint8_t row_index) {
    const FzNoteTextInputKey* row = NULL;
    if(row_index < 3) {
...
static char get_selected_char(FzNoteTextInputModel* model) {
    return get_row(
...
static void fznote_text_input_backspace_cb(FzNoteTextInputModel* model) {
...
static void fznote_text_input_view_draw_callback(Canvas* canvas, void* _model) {
    ...
    for(uint8_t row = 0; row < keyboard_row_count; row++) {
        ...
        const FzNoteTextInputKey* keys = get_row(keyboards[model->selected_keyboard], row);
...
static void
    fznote_text_input_handle_left(FzNoteTextInput* fznote_text_input, FzNoteTextInputModel* model) {
    UNUSED(fznote_text_input);
    if(model->cursor_select) {
...
static void fznote_text_input_handle_right(
    FzNoteTextInput* fznote_text_input,
...
static void fznote_text_input_handle_ok(
    FzNoteTextInput* fznote_text_input,
    FzNoteTextInputModel* model,
    InputType type) {
    ...
    if(selected == ENTER_KEY) {
        if(model->validator_callback &&
           (!model->validator_callback(
            ...
            furi_timer_start(fznote_text_input->timer, furi_kernel_get_tick_frequency() * 4);
        ...
        if(selected == BACKSPACE_KEY) {
            fznote_text_input_backspace_cb(model);
        } else if(!repeat) {
            if(model->clear_default_text) {
                text_length = 0;
            }
...
static bool fznote_text_input_view_input_callback(InputEvent* event, void* context) {
    FzNoteTextInput* fznote_text_input = context;
    furi_assert(fznote_text_input);

    bool consumed = false;

    // Acquire model
    FzNoteTextInputModel* model = view_get_model(fznote_text_input->view);
    ...
    if((!(event->type == InputTypePress) && !(event->type == InputTypeRelease)) &&
       model->validator_message_visible) {
        ...
        switch(event->key) {
        case InputKeyUp:
            fznote_text_input_handle_up(fznote_text_input, model);
            break;
        case InputKeyDown:
            fznote_text_input_handle_down(fznote_text_input, model);
            break;
        case InputKeyLeft:
            fznote_text_input_handle_left(fznote_text_input, model);
            break;
        case InputKeyRight:
            fznote_text_input_handle_right(fznote_text_input, model);
            break;
        case InputKeyOk:
            fznote_text_input_handle_ok(fznote_text_input, model, event->type);
            break;
        default:
            consumed = false;
            break;
    ...
    // Commit model
    view_commit_model(fznote_text_input->view, consumed);

    return consumed;
}

void fznote_text_input_timer_callback(void* context) {
    furi_assert(context);
    FzNoteTextInput* fznote_text_input = context;

    with_view_model(
        fznote_text_input->view,
        FzNoteTextInputModel * model,
        { model->validator_message_visible = false; },
        true);
}
...
FzNoteTextInput* fznote_text_input_alloc() {
    FzNoteTextInput* fznote_text_input = malloc(sizeof(FzNoteTextInput));
    fznote_text_input->view = view_alloc();
    view_set_context(fznote_text_input->view, fznote_text_input);
    view_allocate_model(
        fznote_text_input->view, ViewModelTypeLocking, sizeof(FzNoteTextInputModel));
    view_set_draw_callback(fznote_text_input->view, fznote_text_input_view_draw_callback);
    view_set_input_callback(fznote_text_input->view, fznote_text_input_view_input_callback);

    fznote_text_input->timer =
        furi_timer_alloc(fznote_text_input_timer_callback, FuriTimerTypeOnce, fznote_text_input);
    ...
    fznote_text_input_reset(fznote_text_input);

    return fznote_text_input;
}

void fznote_text_input_free(FzNoteTextInput* fznote_text_input) {
    furi_assert(fznote_text_input);
    with_view_model(
        fznote_text_input->view,
        FzNoteTextInputModel * model,
    ...
    // Send stop command
    furi_timer_stop(fznote_text_input->timer);
    // Release allocated memory
    furi_timer_free(fznote_text_input->timer);

    view_free(fznote_text_input->view);

    free(fznote_text_input);
}

void fznote_text_input_reset(FzNoteTextInput* fznote_text_input) {
    furi_assert(fznote_text_input);
    with_view_model(
        fznote_text_input->view,
        FzNoteTextInputModel * model,
...
View* fznote_text_input_get_view(FzNoteTextInput* fznote_text_input) {
    furi_assert(fznote_text_input);
    return fznote_text_input->view;
}
...
void fznote_text_input_set_minimum_length(
    FzNoteTextInput* fznote_text_input,
    size_t minimum_length) {
    with_view_model(
        fznote_text_input->view,
...
FzNoteTextInputValidatorCallback
    fznote_text_input_get_validator_callback(FzNoteTextInput* fznote_text_input) {
    FzNoteTextInputValidatorCallback validator_callback = NULL;
    with_view_model(
        fznote_text_input->view,
        FzNoteTextInputModel * model,
...
void* fznote_text_input_get_validator_callback_context(FzNoteTextInput* fznote_text_input) {
...
void fznote_text_input_set_header_text(FzNoteTextInput* fznote_text_input, const char* text) {
    with_view_model(
        fznote_text_input->view, FzNoteTextInputModel * model, { model->header = text; }, true);
}
